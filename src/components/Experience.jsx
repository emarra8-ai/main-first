...
export default function Experience() {
  return (
    <section id="experience" className="bg-[#0B0B0B] px-6 py-24 sm:py-28">
      <div className="max-w-4xl mx-auto">
        ...
        <Reveal delay={0.05}>
          <h2 className="font-serif text-4xl sm:text-5xl text-[#FAF6EF] leading-tight mb-14">
        ...
        <div className="relative">
          {/* vertical line */}
          <div className="absolute left-[7px] top-2 bottom-2 w-px bg-white/15" aria-hidden="true" />

          <ol className="space-y-10">
            {TIMELINE.map((item, i) => (
              <Reveal as="li" key={item.place} delay={i * 0.06} className="relative pl-10">
                {/* dot */}
                <span className="absolute left-0 top-1.5 w-[15px] h-[15px] rounded-full bg-[#8E2A24] ring-4 ring-[#0B0B0B]" />
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 mb-1">
                  <span className="text-sm font-semibold uppercase tracking-wide text-[#E0754A]">
                    {item.period}
                  </span>
                  <span className="text-white/30">&middot;</span>
                  <span className="text-sm font-medium text-[#B7ADA2]">
                    {item.place}
                  </span>
                </div>
                <h3 className="text-xl font-semibold text-[#FAF6EF] mb-1.5">
                  {item.title}
                </h3>
                <p className="text-[#B7ADA2] leading-relaxed max-w-2xl">
                  {item.body}
                </p>
              </Reveal>
            ))}
